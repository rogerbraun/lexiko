...
text.each_char do |aktuelles_zeichen|
  ...
  kanji_hash[aktuelles_zeichen] += 1 if aktuelles_zeichen =~ kanji_regex
...
# Hier werden nur die zehn häufigsten Kanji ausgegeben.

puts kanji_hash.sort{|a,b| b[1] <=> a[1]}.map{|kanji,haeufigkeit| "#{kanji} - #{haeufigkeit}"}[0..9]
